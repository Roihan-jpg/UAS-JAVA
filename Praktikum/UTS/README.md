...
public class HospitalQueueSystemQuestion {
    // Menyimpan daftar pasien dalam antrian
    ...
    public static void main(String[] args) {
        ...
        while (running) {
            ...
            int choice = getValidIntInput("Enter your choice: "); // Ambil pilihan user

            switch (choice) {
                case 1:
                    addPatient(); // Tambah pasien ke antrian
                    break;
                case 2:
                    serveNextPatient(); // Layani pasien pertama
                    break;
                case 3:
                    displayQueue(); // Tampilkan semua pasien dalam antrian
                    break;
                case 4:
                    updatePriority(); // Update prioritas pasien
                    break;
                case 5:
    ...
    private static void addPatient() {
        ...
        // Urutkan pasien berdasarkan prioritas (semakin kecil semakin diprioritaskan)
    ...
    private static void serveNextPatient() {
        // Layani pasien terdepan dalam antrian
        System.out.println("\n--- Serve Next Patient ---");
    ...
    private static void updatePriority() {
        // Update prioritas pasien tertentu berdasarkan nama
        ...
        for (Patient p : patientQueue) {
            if (p.getName().equalsIgnoreCase(name)) {
                ...
                found = true;
                // Re-sort antrian setelah update
                patientQueue.sort((p1, p2) -> Integer.compare(p1.getPriority(), p2.getPriority()));
    ...
    private static String getPriorityText(int priority) {
        // Konversi angka prioritas ke dalam teks
        switch (priority) {
            case 1:
                return "1-Critical";
            case 2:
    ...
    private static int getValidIntInput(String prompt) {
        // Validasi input agar hanya menerima angka integer
        int value;
        while (true) {
    ...
    private static int getValidIntInRange(String prompt, int min, int max) {
        // Validasi agar input integer berada dalam rentang tertentu
        int value;
        while (true) {
    ...
    private static String getValidStringInput(String prompt) {
        // Validasi agar input string tidak kosong
        String value;
        while (true) {
            System.out.print(prompt);
